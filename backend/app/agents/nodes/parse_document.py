...
async def parse_document(state: ComplaintState) -> ComplaintState:
    ...
    fields = _demo_extract(state["raw_text"])
    if settings.groq_api_key:
        try:
            ...
            fields = {**fields, **json.loads(response.choices[0].message.content)}
        except Exception as exc:
            state.setdefault("errors", []).append(f"AI extraction fallback used: {exc}")
